...
from config.log_formats import COMMON_TRAIN_FORMAT, COMMON_EVAL_FORMAT, AGENT_TRAIN_FORMAT
...
class Logger(object):
    ...
    def log(self, key, value, step, n=1, log_frequency=1):
        if not self._should_log(step, log_frequency):
            return
        assert key.startswith('train') or key.startswith('eval') or key.startswith('prec')
        if type(value) == torch.Tensor:
            value = value.item()
        self._try_sw_log(key, value / n, step)
        # dont log prec stuff to csv
